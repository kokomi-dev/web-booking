...
const Search = ({
  ...
  const handleSearch = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!value) {
      ...
      router.push(`/searchresult?address=${value}`);
    }
  };
  ...
  return (
    <form className="w-full h-full relative">
      ...
      <div
        className={`w-full px-20 absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] z-[10] flex flex-col items-start justify-center text-white  ${
          page === "search" ? "relative top-[0%] translate-y-[0%]" : "h-full"
        }`}
      >
        {page === "tours" && (
          <div>
        ...
        <div
          className={`w-full flex items-center justify-between gap-5 px-1 ${
            page === "search"
              ? "bg-neutral-300 h-[70px]"
              : "mt-20 bg-red-400 h-[90px]"
          }`}
        >
          <div className="w-[45%] h-[88%] px-3 flex justify-center items-center rounded-sm bg-white relative">
          ...
          {(page === "tours" || page === "search") && (
            <div className="w-[45%] h-[88%] px-3 flex items-center justify-between rounded-sm bg-white">
              <CalendarIcon className="mr-3 text-black_sub h-[1.3rem] w-[1.3rem]" />
              <DatePicker date={date} setDate={setDate} />
            </div>
          )}
          {page === "hotels" && (
            <div className="w-[45%] h-[88%] px-3 flex items-center justify-center rounded-sm bg-white">
          ...
          <div className="w-[10%] h-[88%] rounded-sm">
            <Button
              type="submit"
              variant="default"
